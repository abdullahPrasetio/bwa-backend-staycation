...
describe('API Endpoin Testing',()=>{
    ...
    it('GET booking Page', (done)=>{
        const image=__dirname+'/bukti-bayar.png';
        const dataSample={
                image,
        ...
        .attach('image',fs.readFileSync(dataSample.image),'bukti-bayar.png')
        .end((err,res)=>{
            expect(err).to.be.null;
            expect(res).to.have.status(201);
            expect(res.body).to.be.an('Object');
            expect(res.body).to.have.property('message');
            expect(res.body.message).to.equals('Success Booking');
            expect(res.body).to.have.property('booking');
            expect(res.body.booking).to.have.all.keys('payments','_id','invoice', 'bookingStartDate'
            , 'bookingEndDate','total','itemId','memberId','__v');
